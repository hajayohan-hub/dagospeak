/**
 * DictionarySearch - Dictionnaire intelligent FR↔MG
 * 100% Offline-first, vocalisation FR, recherche bidirectionnelle
 */
export class DictionarySearch {
  #container = null;
  #entries = [];
  #filteredEntries = [];
  #currentLang = 'fr'; // Langue de recherche active
  #currentPage = 0;
  #pageSize = 20;
  #selectedEntry = null;
  ...
  async init() {
    ...
    try {
      ...
      this.#render();
      console.log(`[DictionarySearch] ✅ ${this.#entries.length} entrées chargées`);
    } catch (e) {
  ...
  async #loadAllDictionaries() {
    // Liste des thèmes disponibles dans dictionary/
    ...
    for (const theme of themes) {
      try {
        ...
        // Fichier non trouvé = pas encore créé, on continue
        console.log(`[DictionarySearch] ℹ️ ${theme}.json non trouvé (pas encore créé)`);
      }
    }
  ...
  #renderMainView() {
    ...
    const paginatedEntries = this.#filteredEntries.slice(
    ...
    this.#container.innerHTML = `
      <section style="max-width: 700px; margin: 0 auto; padding: 1rem;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
          ...
          <span style="font-size:0.85rem; color:var(--ds-color-text-muted);">
            ${this.#filteredEntries.length} / ${totalWords} teny (mots)
          </span>
        </div>

        <h2 style="text-align:center; color:var(--ds-color-primary); margin-bottom:0.25rem;">
          📖 Rakibolana (Dictionnaire)
        </h2>
        <p style="text-align:center; color:var(--ds-color-text-muted); font-size:0.9rem; margin-bottom:1.5rem; font-style:italic;">
          Dikanteny FR ↔ MG • Tsindrio ny teny hihainoana
        </p>
        ...
        <div style="position:relative; margin-bottom:1rem;">
          <input type="text" id="dict-search-input" placeholder="Tadiavo... (Rechercher un mot)"
            style="
              ...
              border:2px solid var(--ds-color-border); border-radius:14px;
        ...
        <div style="display:flex; gap:0.5rem; margin-bottom:1.5rem; justify-content:center;">
          <button class="dict-lang-btn ${this.#currentLang === 'fr' ? 'active' : ''}" data-lang="fr" style="
            padding:8px 20px; border-radius:20px; font-weight:600; cursor:pointer; font-size:0.9rem;
            border:2px solid ${this.#currentLang === 'fr' ? 'var(--ds-color-primary)' : 'var(--ds-color-border)'};
            background:${this.#currentLang === 'fr' ? 'var(--ds-color-primary)' : 'var(--ds-color-surface)'};
            color:${this.#currentLang === 'fr' ? 'white' : 'var(--ds-color-text)'};
            transition: all 0.2s;
          ">🇫🇷 Français</button>
          <button class="dict-lang-btn ${this.#currentLang === 'mg' ? 'active' : ''}" data-lang="mg" style="
            padding:8px 20px; border-radius:20px; font-weight:600; cursor:pointer; font-size:0.9rem;
            border:2px solid ${this.#currentLang === 'mg' ? 'var(--ds-color-accent)' : 'var(--ds-color-border)'};
            background:${this.#currentLang === 'mg' ? 'var(--ds-color-accent)' : 'var(--ds-color-surface)'};
            color:${this.#currentLang === 'mg' ? 'white' : 'var(--ds-color-text)'};
            transition: all 0.2s;
          ">🇲🇬 Malagasy</button>
        </div>
        ...
        <div style="display:flex; gap:0.5rem; margin-bottom:1.5rem; overflow-x:auto; padding-bottom:0.5rem;">
          <button class="dict-theme-btn active" data-theme="all" style="
            padding:6px 14px; border-radius:16px; font-size:0.8rem; font-weight:600; cursor:pointer;
            white-space:nowrap; border:1px solid var(--ds-color-primary);
            background:var(--ds-color-primary); color:white;
          ">Tous</button>
          ${[...new Set(this.#entries.map(e => e.category))].map(cat => `
            <button class="dict-theme-btn" data-theme="${cat}" style="
              padding:6px 14px; border-radius:16px; font-size:0.8rem; font-weight:600; cursor:pointer;
              white-space:nowrap; border:1px solid var(--ds-color-border);
              background:var(--ds-color-surface); color:var(--ds-color-text);
            ">${this.#getThemeIcon(cat)} ${cat}</button>
          `).join('')}
        </div>

        <!-- LISTE DES MOTS -->
        <div id="dict-entries-list" style="display:flex; flex-direction:column; gap:0.75rem; margin-bottom:1.5rem;">
          ${paginatedEntries.length > 0 ? paginatedEntries.map(entry => this.#renderEntryCard(entry)).join('') : `
            <div style="text-align:center; padding:2rem; color:var(--ds-color-text-muted);">
        ...
        ${totalPages > 1 ? `
          <div style="display:flex; justify-content:center; gap:0.5rem; margin-bottom:1rem;">
            <button id="dict-prev-page" ${this.#currentPage === 0 ? 'disabled' : ''} style="
              padding:8px 16px; border-radius:8px; border:1px solid var(--ds-color-border);
              background:var(--ds-color-surface); cursor:${this.#currentPage === 0 ? 'not-allowed' : 'pointer'};
              opacity:${this.#currentPage === 0 ? '0.5' : '1'};
            ">←</button>
            <span style="padding:8px 16px; font-weight:600; color:var(--ds-color-text);">
              ${this.#currentPage + 1} / ${totalPages}
            </span>
            <button id="dict-next-page" ${this.#currentPage >= totalPages - 1 ? 'disabled' : ''} style="
              padding:8px 16px; border-radius:8px; border:1px solid var(--ds-color-border);
              background:var(--ds-color-surface); cursor:${this.#currentPage >= totalPages - 1 ? 'not-allowed' : 'pointer'};
  ...
  #renderEntryCard(entry) {
    ...
    return `
      <div class="dict-entry-card" data-id="${entry.id}" style="
        background:var(--ds-color-surface); padding:1rem 1.25rem; border-radius:12px;
        border:1px solid var(--ds-color-border); cursor:pointer;
      ...
      ">
        <div style="flex:1;">
          ...
          <div style="font-size:0.9rem; color:var(--ds-color-text-muted); margin-left:2.25rem;">
            → ${translation}
          </div>
        </div>
        <button class="dict-play-btn" data-tts="${entry.audio?.ttsTextFr || entry.wordFr}" style="
          background:var(--ds-color-primary-soft, #d1fae5); border:none; border-radius:50%;
  ...
  #renderEntryDetail(entry) {
    ...
    this.#container.innerHTML = `
      <section style="max-width: 700px; margin: 0 auto; padding: 1rem;">
        ...
        <!-- EN-TÊTE DU MOT -->
        <div style="
          background: linear-gradient(135deg, var(--ds-color-primary) 0%, #087a62 100%);
          padding: 2rem; border-radius: 16px; margin: 1rem 0 1.5rem 0;
        ...
        ">
          ...
          ${entry.phonetic ? `<p style="margin:0.75rem 0 0 0; font-family:monospace; font-size:1rem; opacity:0.8;">${entry.phonetic}</p>` : ''}

          <button id="btn-play-detail" style="
            margin-top:1rem; background:white; color:var(--ds-color-primary);
        ...
        <div style="background:var(--ds-color-surface); padding:1.5rem; border-radius:12px; border:1px solid var(--ds-color-border); margin-bottom:1rem;">
          ...
          <div style="display:grid; gap:0.5rem;">
            <div style="display:flex; justify-content:space-between; padding:0.5rem 0; border-bottom:1px solid var(--ds-color-border);">
              <span style="color:var(--ds-color-text-muted);">Catégorie</span>
              <strong>${this.#getThemeIcon(entry.category)} ${entry.category}</strong>
            </div>
            <div style="display:flex; justify-content:space-between; padding:0.5rem 0; border-bottom:1px solid var(--ds-color-border);">
              <span style="color:var(--ds-color-text-muted);">Niveau CECR</span>
              <strong>${entry.level}</strong>
            </div>
            <div style="display:flex; justify-content:space-between; padding:0.5rem 0; border-bottom:1px solid var(--ds-color-border);">
              <span style="color:var(--ds-color-text-muted);">Nature du mot</span>
              <strong>${posLabel}</strong>
            </div>
            ${entry.exam?.eligibleFor ? `
              <div style="display:flex; justify-content:space-between; padding:0.5rem 0;">
                <span style="color:var(--ds-color-text-muted);">Examens</span>
                <strong>${entry.exam.eligibleFor.map(e => e.replace('exam_', '').toUpperCase()).join(', ')}</strong>
              </div>
            ` : ''}
          </div>
        </div>
        ...
        <!-- BOUTON RETOUR -->
        <button id="btn-back-list-bottom" style="
          width:100%; padding:14px; background:var(--ds-color-primary);
          color:white; border:none; border-radius:12px;
          font-weight:600; font-size:1rem; cursor:pointer;
        ">← Hiverina amin'ny lisitra (Retour à la liste)</button>
      </section>
    `;

    // Écouteurs
    document.getElementById('btn-back-list')?.addEventListener('click', () => this.#renderMainView());
    document.getElementById('btn-back-list-bottom')?.addEventListener('click', () => this.#renderMainView());
    document.getElementById('btn-play-detail')?.addEventListener('click', () => {
      this.#speakFrench(entry.audio?.ttsTextFr || entry.wordFr, entry.audio?.ttsRate || 0.9);
    });
  }
  ...
  #attachEventListeners() {
    ...
    searchInput?.addEventListener('input', (e) => {
      ...
      searchTimeout = setTimeout(() => {
        this.#filterEntries(e.target.value.trim());
      }, 200); // Debounce 200ms
    });
    ...
    document.querySelectorAll('.dict-lang-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        this.#filterEntries(searchInput?.value.trim() || '');
      });
    });
    ...
    document.querySelectorAll('.dict-theme-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        document.querySelectorAll('.dict-theme-btn').forEach(b => {
          b.style.background = 'var(--ds-color-surface)';
          b.style.color = 'var(--ds-color-text)';
          b.style.borderColor = 'var(--ds-color-border)';
          b.classList.remove('active');
        });
        btn.style.background = 'var(--ds-color-primary)';
        btn.style.color = 'white';
        btn.style.borderColor = 'var(--ds-color-primary)';
        btn.classList.add('active');
        this.#filterEntries(searchInput?.value.trim() || '', btn.dataset.theme);
      });
    });
    ...
    document.querySelectorAll('.dict-entry-card').forEach(card => {
      card.addEventListener('click', (e) => {
        // Ne pas ouvrir le détail si on clique sur le bouton play
  ...
  #filterEntries(query = '', theme = 'all') {
    const activeTheme = theme || (document.querySelector('.dict-theme-btn.active')?.dataset.theme) || 'all';

    this.#filteredEntries = this.#entries.filter(entry => {
      // Filtre par thème
      if (activeTheme !== 'all' && entry.category !== activeTheme) return false;
  ...
  #getThemeIcon(theme) {
    const icons = {
      'market': '🛒', 'family': '👨‍👩‍👧', 'survival': '🆘',
      'numbers': '🔢', 'colors': '🎨', 'days': '📅',
      'months': '🗓️', 'greetings': '👋', 'body': '🧍',
      'alphabet1': '🔤', 'alphabet2': '🔡', 'numbers2': '🧮'
    };
    return icons[theme] || '📁';
  }
}